...
export default function FaceMatchResultScreen({ navigation, route }) {
  ...
  const suspectId =
    responseData?.suspect?.suspect_id || responseData?.suspect?.id || 'S004';

  const suspectName =
    responseData?.suspect?.name || 'Anvi Mishra';

  const suspectStatus =
    responseData?.suspect?.status ||
    (responseData?.suspect?.wanted ? 'WANTED' : 'WANTED');
